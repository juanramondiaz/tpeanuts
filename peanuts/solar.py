...
class SolarModel:
    ...
    def __init__(self, solar_model_file=None, flux_file=None, spectrum_files=None, fluxrows=None, fluxcols=None, fluxscale=None, distrow=None, radiuscol=None, densitycol=None, fractioncols=None):
        ...
        data_path = os.path.join(path, "..", "data", "peanuts")
        # Default solar model file is the B16 from Vinyoles et al 2016, https://www.ice.csic.es/personal/aldos/solar_data.html
        self.solar_model_file = os.path.join(data_path, "nudistr_b16_agss09.dat") if solar_model_file == None else solar_model_file
        # Default flux file is the B16 from Vinyoles et al 2016, https://www.ice.csic.es/personal/aldos/solar_data.html
        self.flux_file = os.path.join(data_path, "fluxes_b16.dat") if (flux_file == None and "b16" in self.solar_model_file) else (solar_model_file if flux_file == None else flux_file)
        ...
        try:
          ...
          for key, val in fluxcols.items():
            self.fluxes[key] = f.read_csv(self.flux_file,
                                   usecols = [val],
                                   names = [key],
                                   sep='\s+', skiprows=fluxrows[key], nrows=1, header=None)[key][0]
            if fluxscale is not None:
          ...
          self.model = f.read_csv(self.solar_model_file,
                                  usecols = distcols,
                                  names = distnames,
                                  sep='\s+', skiprows=distrow, header=None)
        ...
        spectrum_files["8B"] = os.path.join(data_path, "8B_shape_Ortiz_et_al.csv") if "8B" not in spectrum_files else spectrum_files['8B']
        spectrum_files["hep"] = os.path.join(data_path, "hep_shape.csv") if "hep" not in spectrum_files else spectrum_files['hep']
        spectrum_files["pp"] = os.path.join(data_path, "pp_shape.csv") if "pp" not in spectrum_files else spectrum_files['pp']
        spectrum_files["17F"] = os.path.join(data_path, "f17_shape.csv") if "17F" not in spectrum_files else spectrum_files['17F']
        spectrum_files["7Be"] = spectrum_files["7Beground"] = os.path.join(data_path, "be7ground_shape.csv") if "7Beground" not in spectrum_files else spectrum_files['7Beground']
        spectrum_files["7Beexcited"] = os.path.join(data_path, "be7excited_shape.csv") if "7Beexcited" not in spectrum_files else spectrum_files['7Beexcited']
        spectrum_files["13N"] = os.path.join(data_path, "n13_shape.csv") if "13N" not in spectrum_files else spectrum_files['13N']
        spectrum_files["15O"] = os.path.join(data_path, "o15_shape.csv") if "15O" not in spectrum_files else spectrum_files['15O']
    ...
    def export_csv(self, path):
        # -----------------------------
        # EXPORT: model, fluxes, spectra -> CSV
        # -----------------------------
        import pandas as pd
        
        # Carpeta de salida (por defecto junto al código, en ../data/Exports)
        export_dir = os.path.join(path, "data", "Exports")
        os.makedirs(export_dir, exist_ok=True)
        
        # 1) Guardar self.model (DataFrame) -> CSV
        model_csv = os.path.join(export_dir, "solar_model.csv")
        ...
        if isinstance(self.model, pd.DataFrame):
            ...
            raise TypeError(f"self.model no es DataFrame, es {type(self.model)}")
        
        # 2) Guardar self.fluxes (dict) -> CSV con cabecera
        #    (formato: fraction, flux)
        fluxes_csv = os.path.join(export_dir, "fluxes.csv")
        if isinstance(self.fluxes, dict):
            flux_df = (
                pd.DataFrame(list(self.fluxes.items()), columns=["fraction", "flux"])
            ...
            raise TypeError(f"self.fluxes no es dict, es {type(self.fluxes)}")
        
        # 3) Guardar cada espectro -> un CSV independiente
        #    (asegura que cada DF tenga columnas Energy,Spectrum)
        spectra_dir = os.path.join(export_dir, "Spectra")
        os.makedirs(spectra_dir, exist_ok=True)
        
        if isinstance(self.spectra, dict):
            for name, df in self.spectra.items():
                if not isinstance(df, pd.DataFrame):
                    raise TypeError(f"self.spectra['{name}'] no es DataFrame, es {type(df)}")
        
                # nombre de archivo seguro
                safe_name = str(name).replace("/", "_").replace(" ", "_")
                out_csv = os.path.join(spectra_dir, f"spectrum_{safe_name}.csv")
        
                # si por lo que sea no tiene columnas esperadas, renombramos
                # (esto es robusto si cambias el lector más adelante)
                df_to_save = df.copy()
                if list(df_to_save.columns) != ["Energy", "Spectrum"]:
                    # intenta mapear por posición si tiene al menos 2 columnas
                    if df_to_save.shape[1] >= 2:
                        df_to_save = df_to_save.iloc[:, :2]
                        df_to_save.columns = ["Energy", "Spectrum"]
            ...
            raise TypeError(f"self.spectra no es dict, es {type(self.spectra)}")
        
        # (opcional) mensaje de confirmación
        print(f"[EXPORT] model -> {model_csv}")
        print(f"[EXPORT] fluxes -> {fluxes_csv}")
        print(f"[EXPORT] spectra -> {spectra_dir} (1 CSV por espectro)")
...
model = SolarModel()
#print(type(model.spectra['13N']),type(model.fluxes['pp']), type(model.model))
#print(model.frac)
#model.export_csv('G:/Mi unidad/03.Codigo/Physics/TFM/JNeutrino')
