...
class earthdensity:
  ...
  def __init__(self, density_file=None, tabulated_density=False, custom_density=False):
    ...
    if custom_density:
      ...
      with nb.objmode(density_file='string', rj='float64[:]', alpha='float64[:]', beta='float64[:]', gamma='float64[:]', deltas='float64[:,:]'):
        ...
        density_file = path + "/../data/peanuts/earth_density.csv" if density_file == None else density_file
  ...
  def parameters(self, eta):
    ...
    for n in range(len(deltas)):
      ...
      for k1 in range(n+1,len(self.deltas)):
        deltas_prime[n] += binom(k1+3,n+3) * deltas[k1,idx_shells::] * np.sin(eta)**(2*(k1-n))
...
def Pearth_integrated(nustate, density, pmns, DeltamSq21, DeltamSq3l,
                      E, depth, mode="analytical", full_oscillation=False, 
                      antinu=False, lam=-1, d1=0, d2=365, ns=1000,
