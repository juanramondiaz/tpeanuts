...
"""
Input/output utilities for Atmosphere height-flux source datasets.
...
produced by the Atmosphere generation pipeline. Source data, not propagation
physics: nothing here depends on ``medium.atmosphere``, and nothing in
``medium.atmosphere``'s live propagation path (``density``/``evolutor``/
``probability``/``flux``) depends on this module -- it is read only by the
offline MCEq/Honda generators (``external.mceq``/``external.honda``) and
