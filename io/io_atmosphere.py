...
"""
Input/output utilities for atmospheric height-flux datasets.

This module owns the canonical loader for atmospheric MCEq/Honda torch
...
produced by the atmospheric generation pipeline.

Public API
----------
OutputConfig
    Dataclass with the output directory, base filename, dtype, and save flags
    used by atmospheric generators.
build_angle_output_path(...)
    Builds the complete output path for one particle/flavour and atmospheric
...
save_phi_Eh_theta_result(...)
    Saves one atmospheric height-differential flux result to a torch file.
...
class OutputConfig:
    """
    Output configuration for atmospheric height-flux torch files.
    ...
    output_dir: str | PathLike[str] = "atmosphere_height_flux_outputs"
    filename: str = "phi_E_theta_h.pt"
...
def build_angle_output_path(
    ...
    *,
    ...
    """
    Build the full output path for one atmospheric flux file.

    This is the single filename/path builder for atmospheric datasets. It
    ...
    Args:
        ...
        theta_deg: Atmospheric surface-intersection angle in degrees.
...
def build_result_metadata(
    ...
    *,
    ...
    """
    Build metadata saved next to one atmospheric flux payload.

    The metadata is intentionally source-neutral: it describes the common
    atmospheric Phi(E,h) file contract used by both MCEq-derived and
    ...
    Args:
        ...
        alpha_deg: Optional detector angle in degrees.
        theta_deg: Optional atmospheric theta angle in degrees.
    ...
    metadata = {
        "description": (
            "Atmospheric height-differential flux dataset."
        ),
        "relation": (
        ...
        "angle_convention": {
            ...
            "theta_deg": "surface-intersection atmospheric angle",
        },
...
def save_phi_Eh_theta_result(
    ...
    *,
    ...
    """
    Save one atmospheric height-differential flux result.
...
def load_phi_Eh_alpha_theta_from_config(
    ...
    *,
    ...
    Args:
        ...
        theta_deg: Atmospheric theta angle in degrees embedded in the filename.
...
def load_phi_Eh_theta_result(
    ...
    *,
    ...
    """
    Load one atmospheric height-differential flux torch file.
...
def load_directory(
    ...
    *,
    ...
    """
    Load and stack all atmospheric flux torch files in a directory.

    This is the canonical dataset loader for MCEq and Honda atmospheric
    ...
    if len(files) == 0:
        raise FileNotFoundError(
            f"No .pt/.pth files found in: {directory}"
        )
...
def load_phi_E_h_flavours_for_theta(
    ...
    *,
    ...
    atmospheric propagation code. Use load_directory for new code that needs
    the full theta grid.

    Args:
        data_dir: Directory containing atmospheric torch files.
        theta_deg: Requested theta angle in degrees.
