...
\begin{implbox}
\begin{itemize}
  \item \pyfunc{vacuum\_flux\_state(nustate, oscillation, E\_MeV, L\_km,
    flux, spectrum, massbasis, ...)}: adds no new physics -- calls
    \pyfunc{vacuum\_probability\_state} above, then
    \pyfunc{core.common.flux.flux\_state} (\cref{ch:core-common}) to apply
    the flux normalisation and optional spectral weight. This is the
    minimal instance of the flux pattern that every later medium's
    \texttt{flux.py} extends.
  \item \pyfunc{vacuum\_flux\_integrated(nustate, oscillation, E\_MeV,
    L\_km, flux, massbasis, ..., energy\_dim=-2)}: builds
