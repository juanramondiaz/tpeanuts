...
This chapter covers the three supported BSM extensions: non-standard
interactions (NSI) in the 3-flavour sector, a fourth light sterile
neutrino (the ``3+1'' scheme), and the Majorana-neutrino extension. All
three reuse as much as possible of the infrastructure from
\cref{ch:core-common}: NSI only modifies $H_{\rm mat}$, reusing whichever
\pyclass{PMNS} object is otherwise in use; the sterile sector extends the
factorisation $U=O\,U_{\rm red}$ to $4\times4$; and the Majorana extension
right-multiplies $U_{\rm red}$ by a phase factor that is provably inert for
every oscillation probability (\S\ref{sec:bsm-majorana}) --- the only one
of the three that changes nothing about propagation results by
construction. There is no separate \texttt{core.BSM} Hamiltonian-assembly
...
\parbox[t]{3.2cm}{\code{nsi\_\allowbreak{}globalfit\_\allowbreak{}esteban2018}} &
  ...
  allowed-region claim, since correlations and matter conventions matter.} \\[22pt]
...
\begin{notebox}
Counting CP phases for the sterile extension requires care: an
$N$-generation Dirac mixing matrix has $(N-1)(N-2)/2$ independent physical
phases. For $N=4$ (the 3+1 case) this gives exactly \keyresult{three} Dirac
phases ($\delta_{13}$, $\delta_{14}$, $\delta_{24}$); the rotation
$R_{34}$ is always real because the fourth phase can be absorbed into a
redefinition of the charged-lepton fields, and hence sterile presets do not
include \code{delta34\_deg}.
\end{notebox}
...
Depending on whether NSI and/or the neutral-current term are active, the
matter term in the reduced basis takes one of three forms:
\begin{itemize}
  \item \textbf{Without NSI and without the neutral-current term} (the
    default). The matter term needs no basis change at all:
    $O_4\,\diag(V_{CC},0,0,0)\,O_4^\dagger=\diag(V_{CC},0,0,0)$ exactly
    (shown above), so
    \begin{equation}
    ...
    (\pyfunc{hamiltonian\_reduced}, \cref{ch:core-common}), dressed back to
    the flavour basis with $O_4(\cdot)O_4^\dagger$
    (\pyfunc{hamiltonian\_flavour}). This case introduces no approximation
    beyond \cref{ch:core-sm}: the reduced-basis simplification stays exact
    for any $\theta_{14},\theta_{24},\theta_{34}$.
  \item \textbf{Without NSI but with the neutral-current term.} Supplying a
    neutron density activates
    $H_{\rm mat,4}^{\rm flavour}=\diag(V_{CC},0,0,-V_{NC})$
    (\cref{ch:core-common}) instead: since this no longer commutes with
    $O_4$, the shortcut above is unavailable and
  ...
  \item \textbf{Simultaneously with NSI (and, optionally, the
    neutral-current term).} Fully supported by the same code path
    ...
    is an $\mathrm{SU}(2)_L\times\mathrm{U}(1)_Y$ singlet and carries no NSI
    coupling), and \pyfunc{hamiltonian\_matter\_reduced} rotates the
    resulting
    $H_{\rm mat,4}^{\rm NSI,\,flavour}=V\bigl(\diag(1,0,0,0)+\epsilon_4\bigr)$
    (plus the $-V_{NC}$ sterile-diagonal term when a neutron density is also
    supplied) into the reduced basis with the \emph{same}
    $O_4^\dagger(\cdot)O_4$ transformation:
    \begin{equation}
    ...
    trade-off as the 3-flavour NSI case applies here too (previous
    sections): the closed-form real-arithmetic shortcut is lost, and
    diagonalisation falls back to \code{torch.linalg.eigh}.
\end{itemize}
...
off \pyclass{OscillationParameters}, so all four combinations (SM, SM+NSI,
...
Global 3+1 fit: \cite{GariazzoGiuntiLavederLi2017}. IceCube atmospheric
...
\parbox[t]{3.2cm}{\code{sterile\_\allowbreak{}3p1\_\allowbreak{}two\_\allowbreak{}flavour\_\allowbreak{}limit}} &
  ...
  \texttt{notebooks/physics/experimental\_results/BSM}).} \\[42pt]
\bottomrule
\end{tabular}
\end{center}
\end{implbox}
...
\section{\texttt{bsm\_majorana.py} --- Majorana neutrinos}
\label{sec:bsm-majorana}

\modulehead{core/BSM/bsm\_majorana.py}{\pyclass{MajoranaPhases} and
\pyclass{PMNS\_Majorana}, the Majorana-neutrino extension of the 3-flavour
PMNS matrix.}

\begin{theorybox}
Whether neutrinos are Dirac or Majorana particles is itself a BSM question,
so this extension lives here rather than in \cref{ch:core-sm} or as a field
on \pyclass{PMNSParams} --- see \S\ref{sec:majorana-phases} for the
rephasing-invariance argument establishing that Majorana phases have no
effect on any oscillation probability. \pyclass{PMNS\_Majorana} is a
\emph{sibling} of \pyclass{PMNS\_SM} (both direct \pyclass{PMNS}
subclasses, exactly mirroring how \pyclass{PMNS\_sterile} sits alongside
\pyclass{PMNS\_SM} rather than inheriting from it), reusing the shared
generators $R_{12}$, $R_{13}$, $R_{23}$, $\Delta$ unchanged and building
\begin{equation}
  \keyresult{U = R_{23}\,\Delta\,R_{13}\,\Delta^\dagger\,R_{12}\,P},
  \qquad
  U_{\rm red} = R_{13}\,R_{12}\,P,
\end{equation}
with $P=\diag(1,e^{i\alpha_{21}/2},e^{i\alpha_{31}/2})$ built from the
companion \pyclass{MajoranaPhases} parameter object (mirroring
\pyclass{PMNSSterileParams}), reusing the already-generic
\pyfunc{PMNS.\_phase\_diag} builder that also constructs $\Delta$ --- no
change to \cref{ch:core-common} was needed. The outer block
$O=R_{23}\,\Delta$ is identical to the plain SM case: the Majorana phase
lives entirely on the mass-eigenstate (right-hand) side of
$U=O\,U_{\rm red}$, so it never touches the matter-Hamiltonian
factorisation.

\textbf{A no-op everywhere it is used, by construction.} Because
\pyclass{PMNS\_Majorana} is a drop-in replacement for \pyclass{PMNS\_SM},
every consumer of a mixing object --- \pyfunc{hamiltonian\_reduced}/%
\pyfunc{hamiltonian\_flavour}, every evolutor
(\cref{ch:core-numerical,ch:core-perturbative}), every probability/flux
function --- produces exactly the same oscillation result whether
\pyclass{PMNS\_SM} or \pyclass{PMNS\_Majorana} is passed in with the same
angles. Indeed this holds already at the Hamiltonian level, not only for
the full oscillation probability: since $P$ is diagonal, it commutes with
the diagonal kinetic-eigenvalue matrix, so
$U_{\rm red}\diag(k)U_{\rm red}^\dagger
=U_{\rm red}^{\rm Dirac}\,P\diag(k)P^\dagger\,{U_{\rm red}^{\rm Dirac}}^\dagger
=U_{\rm red}^{\rm Dirac}\diag(k){U_{\rm red}^{\rm Dirac}}^\dagger$
identically, for any $\alpha_{21},\alpha_{31}$.

\textbf{Configuration attribute.}
\pyfunc{PropagationConfig.oscillation\_parameters\_from\_preset}
(\cref{ch:core-common}) exposes the Dirac/Majorana choice as a single
optional argument, \code{neutrino\_nature\allowbreak{}$\in\{$}\code{"dirac"}
(default)\code{, "majorana"}\allowbreak{}\code{\}}, together with
\code{alpha21\_deg}/\code{alpha31\_deg}. \code{"dirac"} builds a plain
\pyclass{PMNS\_SM} --- every propagation result is exactly as it was before
this extension existed. \code{"majorana"} builds a \pyclass{PMNS\_Majorana}
instead; \pyclass{OscillationParameters.BSM\_extension\_majorana}
(duck-typed on \code{hasattr(pmns, "majorana\_params")}, exactly like
\pyclass{BSM\_extension\_NSI} is duck-typed on \code{nsi}) reflects this
choice without \cref{ch:core-common} needing to import this module.
Combining \code{neutrino\_nature="majorana"} with a 3+1 sterile preset
raises \code{ValueError}: \pyclass{PMNS\_sterile} does not (yet) apply the
Majorana phase factor.

\textbf{The one observable that does depend on it.}
\pyfunc{effective\_majorana\_mass} computes the effective Majorana mass
\begin{equation}
  \keyresult{m_{\beta\beta} = \left|\sum_i U_{ei}^2\,m_i\right|},
\end{equation}
governing the rate of neutrinoless double-beta decay. Unlike an
oscillation amplitude, this is \emph{quadratic} (not linear) in the
electron row of $U$, so the Majorana phases do \emph{not} cancel here ---
this is the standard textbook counter-example to the rephasing-invariance
argument above. It is a zero-baseline, purely algebraic quantity: no
dependence on $L$, $E$, a matter profile, or a propagation method, and it
requires the \emph{absolute} mass scale rather than only the mass-squared
splittings this project otherwise ever needs
(\pyfunc{MassSpectrum.absolute\_mass\_squared\_vector},
\cref{ch:core-common}, \texttt{mass\_spectrum.py} section). This module is
deliberately isolated: nothing in \texttt{core.numerical},
\texttt{core.perturbative}, or \texttt{medium.*} imports it, and it imports
none of them.
\end{theorybox}

\begin{implbox}
\begin{itemize}
  \item \pyclass{MajoranaPhases}: immutable container for
    $\alpha_{21},\alpha_{31}$ and a \pyclass{RuntimeContext} --- a
    companion to a \pyclass{PMNSParams} instance, exactly like
    \pyclass{PMNSSterileParams}; it does not modify \pyclass{PMNSParams}
    itself.
  \item \pyclass{PMNS\_Majorana(sm\_params, majorana\_params)}: mirrors
    \pyclass{PMNS\_sterile(sm\_params, sterile\_params)}'s constructor
    signature. Implements \pyfunc{outer\_block} (identical to
    \pyclass{PMNS\_SM}), \pyfunc{reduced}, and \pyfunc{pmns\_matrix}
    (both right-multiplied by \pyfunc{\_majorana\_phase\_matrix}).
  \item \pyfunc{effective\_majorana\_mass(pmns\_matrix,
    mass\_squared\_vector\_eV2)}: reads the electron row (index 0) and the
    first three columns of \code{pmns\_matrix}, returning
    $m_{\beta\beta}$ in eV. Works identically whether \code{pmns\_matrix}
    came from \pyclass{PMNS\_SM} (Dirac; a phase-convention-dependent
    number with no BSM content) or \pyclass{PMNS\_Majorana}.
\end{itemize}
\end{implbox}

\begin{notebox}
\textbf{Performance note.} Because
\pyclass{OscillationParameters.BSM\_extension} is \code{True} whenever
\emph{any} BSM extension is active, opting into
\code{neutrino\_nature="majorana"} routes perturbative-evolutor
construction through the general BSM code path
(\cref{ch:core-perturbative}) rather than the SM fast path, even though
the identity above proves the two produce the same Hamiltonian. This is an
accepted, already-established tradeoff (an SM+NSI configuration with
$\epsilon=0$ takes the same general path today) rather than a new one
introduced by this extension. For the same reason, the solar
\code{method="adiabatic\_approximated"} closed-form path
(\cref{sec:bsm-support-matrix}) currently rejects
\code{neutrino\_nature="majorana"} together with every other BSM
extension, even though it is physically exact for it too --
\code{method="adiabatic\_exact"} or \code{method="numerical"} remain
available and unaffected.
\end{notebox}

\section{Supported configurations across the codebase}
\label{sec:bsm-support-matrix}

This chapter and \cref{ch:core-sm} establish the three orthogonal
oscillation scenarios exposed by \pyclass{OscillationParameters}: the plain
Standard Model, the Non-Standard Interactions extension, and the 3+1
sterile extension --- freely combinable, in any pair or all three at once,
via \code{oscillation.pmns.n\_flavours} and \code{oscillation.nsi}
(\cref{ch:core-common}). Which of these combinations a given
\emph{propagation method} actually accepts, however, is not uniform:
\pyfunc{hamiltonian\_reduced} / \pyfunc{hamiltonian\_flavour} themselves, the
generic numerical evolutor, and most perturbative and solar paths are fully
generic in flavour count and NSI content, while a couple of closed-form
solar methods are deliberately restricted to the plain SM because their
underlying formula has no BSM generalisation. This section collects that
support matrix in one place, so a caller can check at a glance whether a
chosen propagation method accepts the oscillation scenario it needs.
...
\midrule
\parbox[t]{4.0cm}{Numerical propagation (\pyfunc{evolutor\_numerical})} &
  \parbox[t]{3.2cm}{\checkmark\ Supported} &
  \parbox[t]{3.2cm}{\checkmark\ Supported} &
  \parbox[t]{3.2cm}{\checkmark\ Supported} \\[12pt]
\parbox[t]{4.0cm}{Perturbative propagation --- vacuum (\texttt{medium.\allowbreak vacuum.\allowbreak evolutor})} &
  \parbox[t]{3.2cm}{\checkmark\ own exact closed form} &
  \parbox[t]{3.2cm}{\checkmark\ (no physical effect: $H_{\rm mat}=0$ in vacuum)} &
  \parbox[t]{3.2cm}{\checkmark\ same closed form, generic in $n$} \\[18pt]
\parbox[t]{4.0cm}{Perturbative propagation --- earth (\texttt{medium.\allowbreak earth.\allowbreak evolutor})} &
  \parbox[t]{3.2cm}{\checkmark\ } &
  \parbox[t]{3.2cm}{\checkmark} &
  \parbox[t]{3.2cm}{\checkmark} \\[18pt]
\parbox[t]{4.0cm}{Perturbative propagation --- atmosphere (\texttt{medium.\allowbreak atmosphere.\allowbreak evolutor})} &
  \parbox[t]{3.2cm}{\checkmark\ } &
  \parbox[t]{3.2cm}{\checkmark} &
  \parbox[t]{3.2cm}{\checkmark} \\[18pt]
\parbox[t]{4.0cm}{Solar --- adiabatic approximated } &
  \parbox[t]{3.2cm}{\checkmark\ } &
  \parbox[t]{3.2cm}{$\times$\ raises \code{ValueError}} &
  \parbox[t]{3.2cm}{$\times$\ raises \code{ValueError}} \\[18pt]
\parbox[t]{4.0cm}{Solar --- adiabatic exact } &
  \parbox[t]{3.2cm}{\checkmark} &
  \parbox[t]{3.2cm}{\checkmark} &
  \parbox[t]{3.2cm}{\checkmark\ (+ optional $V_{NC}$)} \\[18pt]
\parbox[t]{4.0cm}{Solar --- fully numerical } &
  \parbox[t]{3.2cm}{\checkmark} &
  \parbox[t]{3.2cm}{\checkmark} &
  \parbox[t]{3.2cm}{\checkmark\ (+ optional $V_{NC}$)} \\[18pt]
\parbox[t]{4.0cm}{Solar --- Landau-Zener correction } &
  \parbox[t]{3.2cm}{\checkmark\ (adiabatic approximated)} &
  \parbox[t]{3.2cm}{$\times$} &
  \parbox[t]{3.2cm}{$\times$} \\[18pt]
\parbox[t]{4.0cm}{Eigenvalue solver, analytic } &
  \parbox[t]{3.2cm}{Cardano ($n=3$)} &
  \parbox[t]{3.2cm}{Cardano ($n=3$) or Ferrari ($n=4$)} &
  \parbox[t]{3.2cm}{Ferrari ($n=4$)} \\[14pt]
\parbox[t]{4.0cm}{Neutral-current term $V_{NC}$ (sterile diagonal) } &
  \parbox[t]{3.2cm}{n/a (unobservable common phase, never computed)} &
  \parbox[t]{3.2cm}{n/a unless sterile also active} &
  \parbox[t]{3.2cm}{\checkmark\ optional ( off by default)} \\[6pt]
\bottomrule
\end{tabular}
\end{center}

\begin{notebox}[title={Reading This Table}]
\begin{itemize}
  \item \textbf{A \checkmark\ is scenario-exact, not error-free.} Every
    \checkmark\ entry above is exact within whatever approximation the
    method itself represents (the adiabatic approximation, a finite
    numerical step size, \dots) --- it is not a claim that the method is
    free of the usual numerical or physical approximations documented in
    its own chapter.
  \item \textbf{Vacuum never touches \cref{ch:core-perturbative} or
    \cref{ch:core-numerical}.} With $H_{\rm mat}=0$ there is nothing to
    perturb or exponentiate segment by segment, so
    \texttt{medium.vacuum.evolutor} implements the exact three-step
    mass-basis scheme directly (\cref{ch:core-sm}, the \textbf{Coherent
    propagation scheme} box). That scheme happens to be $n$-flavour
    generic, so NSI (a no-op there) and the sterile extension are
    ``supported'' in the sense that nothing breaks, not because the
    perturbative or numerical machinery was invoked.
  \item \textbf{The solar adiabatic-approximated method and Landau-Zener are
    the only genuinely SM-only propagation paths in the whole project.}
    Both rely on a closed-form two-level mixing-angle formula
    ($\theta_{12}^M$, $\theta_{13}^M$, and the MSW resonance condition
    respectively) that has no NSI or 3+1 generalisation, and the solar
    dispatcher enforces this by raising rather than silently ignoring the
    BSM extension.
  \item \textbf{The analytic eigenvalue solver is selected by flavour count
    alone, never by whether NSI is active.} Cardano's trigonometric cubic
    solution applies to any traceless Hermitian $3\times3$ reduced
    Hamiltonian (SM or SM+NSI), and Ferrari's quartic solution applies to
    any traceless Hermitian $4\times4$ one (sterile, with or without NSI or
    $V_{NC}$) --- both formulas only require Hermiticity, not a real
    matrix, so neither NSI's complex $\epsilon$ nor a non-zero
    active-sterile CP phase $\delta_{14}$ forces a numerical fallback. That
    fallback (\code{torch.linalg.eigvalsh}) is used only when
    \code{analytic\_eigenvalues=False} (the default) or when the
    closed-form solver's own conditioning guard detects a near-degenerate
    spectrum.
\end{itemize}
\end{notebox}
