...
\begin{itemize}
  ...
  \item The teal box \textbf{Usage Example} (from \cref{ch:pipeline-solar}
    onwards) gives a complete, runnable Python snippet showing how the
    module's public entry points are actually called together, with prose
...
\end{equation}
\begin{itemize}
  \item In vacuum each mass eigenstate evolves with a kinetic phase $e^{-iE_i L}$.
  \item When crossing matter an additional effective potential from coherent forward
interaction appears (the MSW effect,\parencite{Wolfenstein1978,MikheyevSmirnov1985}).
\end{itemize}
In both cases the evolution is summarised by an effective Hamiltonian $H(x)$ acting
 on the vector of flavour amplitudes,
\begin{equation}
