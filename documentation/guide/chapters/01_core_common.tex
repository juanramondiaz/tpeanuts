...
Standard Model, $V_{NC}\,\mathbb{I}_3$ is a common phase across all three
...
Hamiltonian, and why $n_n$ is nowhere plumbed through the SM code path. 

The 3+1 sterile extension changes this qualitatively, since the sterile flavour
is a gauge singlet and \emph{does not} receive $V_{NC}$: subtracting the
same global phase $V_{NC}\,\mathbb{I}_4$ from
$\diag(V_{CC}+V_{NC},\,V_{NC},\,V_{NC},\,0)$ leaves a genuine relative term
on the sterile diagonal entry,
\begin{equation}
  \diag(V_{CC}+V_{NC},\,V_{NC},\,V_{NC},\,0) - V_{NC}\,\mathbb{I}_4
...
CP phase) without modifying this base class at all (BSM chapter). 

How $R_{12}$, $R_{13}$, $R_{23}$, $\Delta$ combine into a full mixing matrix ---
...
\subsection*{Majorana phases (a BSM concept, not part of this container)}
\label{sec:majorana-phases}
...
lepton-number-violating mass term at all --- so it is deliberately \emph{not}
part of \pyclass{PMNSParams} or \pyclass{PMNS\_SM}: it is implemented as a
third BSM extension, \pyclass{PMNS\_Majorana}, alongside NSI and the 3+1
sterile scheme (\cref{ch:core-bsm}). The physics is summarised here because
the underlying rephasing-invariance argument is genuinely
core.common-level: if neutrinos are Majorana particles, the PMNS matrix
carries two additional physical phases beyond $\delta$, conventionally
attached to mass eigenstates 2 and 3,
\begin{equation}
  U = U_{\rm Dirac}\,P,
  \qquad
  P = \diag\!\left(1,\;e^{i\alpha_{21}/2},\;e^{i\alpha_{31}/2}\right),
\end{equation}
with $U_{\rm Dirac}$ the ordinary Dirac-case matrix built from
$\theta_{12},\theta_{13},\theta_{23},\delta$ above. These phases have
\emph{no effect on any oscillation probability}: in the amplitude
$A(\nu_\alpha\to\nu_\beta)=\sum_iU_{\beta i}U_{\alpha i}^* e^{-ik_ix}$, the
extra factor $P_{ii}$ contributed by $U_{\beta i}$ cancels exactly against
$P_{ii}^*$ from $U_{\alpha i}^*$, for every $i$, independent of $k_i$, of
the matter profile, and of the propagation method. This is a
rephasing-invariance statement, not an approximation, and it holds for
every pipeline in this project, since all of them build their amplitudes
from whichever $U$ \pyfunc{pmns\_matrix}/\pyfunc{reduced} returns ---
\pyclass{PMNS\_Majorana} is a drop-in replacement for \pyclass{PMNS\_SM}
precisely because of this. See \cref{ch:core-bsm} for
\pyclass{PMNS\_Majorana}, the \code{neutrino\_nature} configuration
attribute, and the effective Majorana mass
$m_{\beta\beta}=\bigl|\sum_iU_{ei}^2m_i\bigr|$, the one observable in this
project that genuinely distinguishes the two cases.
\end{theorybox}
...
\begin{itemize}
  ...
  \item \pyfunc{difference\_vector()}: abstract; sizes the base vector to
    ...
    not.
  \item Consumed by \pyfunc{kinetic\_eigenvalue\_vector} (\texttt{hamiltonian.py},
    next section) to build the dimensionless kinetic phases $k_i$, and bundled into
...
\begin{implbox}
This introduces no new physics: it bundles \code{pmns}, the mass-squared
splittings, the antineutrino selector, and the optional NSI coupling that
the propagation stack needs into a single immutable object, rather than
threading them as independent arguments through every function.
\pyclass{OscillationParameters} is a passive domain container: it does not
build itself, and it never imports the
...
\code{preset\_name} & \code{str} & \parbox[t]{2.1cm}{\code{"\_SM\_\allowbreak NUFIT52\_\allowbreak NO"}} & \parbox[t]{4.4cm}{Name of the preset used to build this object; \code{"custom"} for the bare dataclass default (explicit construction, no preset).} \\[10pt]
\code{ordering} & \code{str} & \code{"NO"} & \parbox[t]{4.4cm}{Mass-ordering label carried by the preset (\code{"NO"}/\code{"IO"}); \code{""} for the bare dataclass default.} \\[2pt]
...
$n$-flavour evolution problem reduces to:
\begin{enumerate}
  \item solving the evolution of the smaller, cheaper reduced Hamiltonian
    $H_{\rm red}$, obtaining $S_{\rm red}(x)$;
  \item \emph{dressing} the result once with the constant matrix $O$ (which
    depends on neither density nor $x$),
...
\end{equation}
Each dimensionless kinetic eigenvalue $k_i$ entering $\diag(k_1,\dots,k_n)$
is obtained by feeding the corresponding component of that mass-squared
vector, together with the neutrino energy $E$, through
\pyfunc{kinetic\_potential} (\texttt{potential.py} section above); applying
this conversion to the whole vector at once, after first sizing
\pyclass{MassSpectrum.difference\_vector()} to \code{oscillation.pmns}'s
flavour count, is exactly what \pyfunc{kinetic\_eigenvalue\_vector} (below)
does.
...
\begin{notebox}[title={Hermitian Conjugate vs. Plain Transpose}]
...
$U_{\rm full}=O\,U_{\rm red}$ per \labelcref{eq:H-factorization}.
...
only becomes observable once $U_{\rm red}$ is genuinely complex. 
...
It is only once \texttt{tpeanuts} generalises to a genuinely 
...
\begin{itemize}
  ...
  \item \pyfunc{compose\_segment\_evolutors(segments, *, segment\_dim=-3,
    multiply="left")}: performs the ordered product above; internally uses a
    binary-tree reduction (\code{util.math.tree\_reduce\_matmul}) to minimise
    the depth of the matrix-multiplication chain on GPU. The default
    \code{multiply="left"} accumulates each new segment on the left
    ($U_{\rm total}\to U_{\rm seg}\,U_{\rm total}$), reproducing the
    ``most-recent-leg-on-the-left'' convention above; \code{multiply="right"}
    accumulates in the opposite (left-to-right) order, for callers that need
    the reversed product directly instead of composing and reversing
    afterwards.
\end{itemize}
\end{implbox}
...
\end{equation}
\begin{itemize}
  \item For a coherent initial state $\psi_\alpha$ (flavour superposition with
...
\end{equation}
\item For an incoherent mass mixture with weights $w_i\geq0$, $\sum_i w_i=1$
...
\code{(..., E, N)} flux ($N=3$ or $4$, the latter for the 3+1 sterile
extension); an angle-resolved \code{(..., E, angle, N)} flux is
